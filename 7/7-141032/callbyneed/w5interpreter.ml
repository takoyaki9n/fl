...
let rec eval_expr env = function
  ...
  | EVar (Name v) -> 
     (try
	 let cntr = List.assoc (Name v) env in
	 valuate cntr
       with
       |Not_found -> raise (Eval_error ("unbound variable " ^ v)))
  | EFun (x, e) -> VFun (x, e, env)
  | ENil -> VNil
  | ECons (e1, e2) -> VCons (ref (CntThunk (e1, ref env)), ref (CntThunk (e2, ref env)))
  | ETup l -> VTup (List.map (fun ex -> ref (CntThunk (ex, ref env))) l)
  | EAdd (e1, e2) -> 
  ...
  | ELet (n, e1, e2) ->
     let env = add_env n (CntThunk (e1, ref env)) env in
     eval_expr env e2
  | ERLets (lets, e) ->
     let envr = ref env in
     envr := List.fold_right (fun (n, ex) ev -> add_env n (CntThunk (ex, envr)) ev) lets env;
     eval_expr !envr e
  | EApp (e1, e2) ->
     (match (eval_expr env e1) with
      | VFun (n, b, e) -> 
	 let e = add_env n (CntThunk (e2, ref env)) e in
	 eval_expr e b
      | _ -> raise (Eval_error "app: applying to not a function"))
  | EMatch (ex, cases) ->
     let (bnd, ex) = find_match (ref (CntThunk (ex, ref env))) cases in
...
and add_env name cnt env =
  (name, ref cnt)::env

and cat_env env1 env2 =
  env1 @ env2

and find_match cntr = function
  | [] -> raise (Eval_error "match failure")
  | (pat, ex)::cases ->
     (match (matching cntr pat) with
      | None -> find_match cntr cases
      | Some bnd -> (bnd, ex))

and matching cntr p =
  match p with
  | PConst c ->
     let v = valuate cntr in
     if v = c then Some empty_env else None
  | PVar n ->
     Some (add_env n !cntr empty_env)
  | PNil ->
     let v = valuate cntr in
     if v = VNil then Some empty_env else None
  | PCons (p1, p2) ->
     (match (valuate cntr) with
      | VCons (cntr1, cntr2) -> 
	 (match (matching cntr1 p1), (matching cntr2 p2) with
	  | Some bnd1, Some bnd2 -> Some (cat_env bnd1 bnd2)
	  | _, _ -> None)
      | _ -> None)
  | PTup pats ->
     (match (valuate cntr) with
      | VTup cntrs -> match_tup cntrs pats
      | _ -> None)
  | _ -> None

and match_tup cntrs pats =
  match cntrs, pats with
  | [], [] -> 
     Some Syntax.empty_env
  | cntr::cntrs, pat::pats -> 
     match (matching cntr pat), (match_tup cntrs pats) with
     | Some bnd1, Some bnd2 -> Some (cat_env bnd1 bnd2)
     | _, _ -> None;;
